my_optim2 <- function(y, x = rep(1/length(y), length(y)), gini.e, pc.inc = NULL,
                     se.gmm = FALSE, se.nls = FALSE, se.scale = FALSE, N = NULL,
                     nrep = 10^3, grid = 1:20, rescale = 1000, gini = FALSE) {
  # Checking validity of inputs
  check_inputs(y = y, x = x, gini.e = gini.e,
               pc.inc = pc.inc, rescale = rescale,
               grid = grid)
...
check_inputs <- function(y, x, gini.e, pc.inc, rescale, grid) {
